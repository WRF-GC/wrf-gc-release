...
!
  ...
  REAL(hp), POINTER, PUBLIC   :: HCO_FRAC_OF_PBL(:,:,:)
  ...
  REAL(fp), POINTER, PUBLIC   :: SUMCOSZA(:,:)
!
! !DEFINED PARAMETERS:
!
...
!
    ...
    IF ( Phase == 2 .AND. IsEmisTime ) THEN
       ...
       !--------------------------------------------------------------------
       IF ( notDryRun ) THEN
          CALL ExtState_SetFields( State_Chm, State_Grid, State_Met, &
...
! !INTERFACE:
!
  SUBROUTINE ExtState_SetFields( State_Chm, State_Grid, State_Met, HcoState, ExtState, RC )
!
! !USES:
!
...
!
    ...
    ! SAVEd scalars for each domain, so ExtState is initialized for all WRF domains (hplin, 5/22/20)
    LOGICAL, DIMENSION(1:8), SAVE :: FIRST_PERDOM = .TRUE.
#endif
...
#if defined( MODEL_WRF )
    ! FIRST = FIRST_PERDOM( State_Grid%ID )
    FIRST = .true. ! Two-domain hack
#endif
    ...
    ! WRF-GC environment: first call is for each individual domain
#if defined( MODEL_WRF )
    FIRST_PERDOM( State_Grid%ID ) = FIRST
#endif
